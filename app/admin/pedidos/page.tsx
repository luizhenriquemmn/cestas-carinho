...
export default function PedidosPage() {
  ...
  const dashboard = useMemo(() => {
    ...
    return {
      ...
      itemCount: valid.reduce((sum, pedido) => sum + pedido.pedido_itens.reduce((itemSum, item) => itemSum + item.quantidade, 0), 0),
    }
  }, [filteredPedidos])

  async function exportOrders() {
    const XLSX = await import("xlsx")
    const rows = filteredPedidos.flatMap((pedido) => pedido.pedido_itens.length ? pedido.pedido_itens.map((item) => ({
      Pedido: pedido.id, Data: dayjs(pedido.created_at).format("DD/MM/YYYY"), Cliente: pedido.clientes?.nome ?? "",
      Telefone: pedido.clientes?.telefone ?? "", Produto: item.produto_snapshot?.nome ?? item.produtos?.nome ?? item.produto_id,
      Quantidade: item.quantidade, "Preço unitário": Number(item.preco_unitario), "Total do item": Number(item.preco_unitario) * item.quantidade,
      Subtotal: Number(pedido.subtotal ?? pedido.total), Desconto: Number(pedido.desconto_valor ?? 0), Entrega: Number(pedido.taxa_entrega ?? 0),
      Total: Number(pedido.total), Status: STATUS_LABELS[pedido.status] ?? pedido.status,
      "Tipo de entrega": pedido.tipo_entrega === "retirada" ? "Retirada" : "Entrega", "Comentário da decisão": pedido.decisao_comentario ?? "",
    })) : [{ Pedido: pedido.id, Data: dayjs(pedido.created_at).format("DD/MM/YYYY"), Cliente: pedido.clientes?.nome ?? "", Produto: "Pedido sem itens", Total: Number(pedido.total), Status: STATUS_LABELS[pedido.status] ?? pedido.status }])
  ...
  function expandedRow(pedido: PedidoComCliente) {
    ...
    return <div className="grid gap-6 lg:grid-cols-2 py-2">
      <div className="space-y-3 text-sm text-gray-600">
        ...
        {pedido.pedido_itens.length ? pedido.pedido_itens.map((item) => <div key={item.id} className="flex items-center justify-between gap-4"><span>{item.produto_snapshot?.nome ?? item.produtos?.nome ?? item.produto_id} × {item.quantidade}</span><span className="flex items-center gap-2"><span>{formatPrice(item.preco_unitario * item.quantidade)}</span><Button type="text" size="small" title="Ver detalhes do produto" icon={<Eye className="w-4 h-4" />} onClick={() => setProductDetail(item)} /></span></div>) : <p className="text-red-600">Pedido sem itens cadastrados</p>}
      </div>
  ...
  return <div className="space-y-5">
    ...
    <Modal title="Detalhes do produto no pedido" open={Boolean(productDetail)} footer={<Button onClick={() => setProductDetail(null)}>Fechar</Button>} onCancel={() => setProductDetail(null)} width={650}>
      {productDetail && productInfo && <div className="grid sm:grid-cols-[180px_1fr] gap-5"><div>{productInfo.foto_url ? <Image src={productInfo.foto_url} alt={productInfo.nome ?? "Produto"} width={180} height={180} style={{ objectFit: "cover", borderRadius: 8 }} /> : <div className="w-[180px] h-[180px] rounded-lg bg-gray-100 flex items-center justify-center text-gray-400">Sem foto</div>}</div><div className="space-y-3 text-sm"><h3 className="text-lg font-semibold text-gray-900">{productInfo.nome ?? productDetail.produto_id}</h3>{productInfo.categoria && <p><strong>Categoria:</strong> {productInfo.categoria}</p>}{productInfo.descricao && <p className="text-gray-600">{productInfo.descricao}</p>}<div className="grid grid-cols-2 gap-2"><p><strong>Quantidade:</strong> {productDetail.quantidade}</p><p><strong>Preço unitário:</strong> {formatPrice(productDetail.preco_unitario)}</p></div><p className="text-base"><strong>Total do item:</strong> {formatPrice(productDetail.preco_unitario * productDetail.quantidade)}</p>{productInfo.itens && productInfo.itens.length > 0 && <div><strong>Composição vendida:</strong><ul className="list-disc pl-5 mt-1 text-gray-600">{productInfo.itens.map((item, index) => <li key={`${item}-${index}`}>{item}</li>)}</ul></div>}{productDetail.produto_snapshot && <Tag color="blue">Dados preservados no momento da compra</Tag>}</div></div>}
    </Modal>
