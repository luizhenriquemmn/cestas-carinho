...
export default function RedefinirSenhaClientePage() {
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-8">
      <h1 className="font-serif text-2xl font-bold text-foreground mb-2">Criar nova senha</h1>
      <p className="text-muted-foreground text-sm mb-6">Use pelo menos 8 caracteres.</p>
      {status === 'checking' ? (
        <p className="text-sm text-center text-muted-foreground">Validando o link de recuperação...</p>
      ) : status === 'invalid' ? (
        <div className="space-y-4 text-center">
          <p className="text-sm text-destructive">Este link é inválido, já foi utilizado ou expirou.</p>
          <p className="text-sm text-muted-foreground">Solicite um novo link e utilize somente o e-mail mais recente.</p>
          <Link href="/conta/recuperar-senha"><Button className="w-full">Solicitar novo link</Button></Link>
          <p><Link href="/conta/login" className="text-sm text-primary hover:underline">Voltar ao login</Link></p>
        </div>
      ) : (
